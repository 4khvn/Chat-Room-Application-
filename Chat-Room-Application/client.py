...
class Client:
    def __init__(self,server_ip, port, client_ip):
        ...
        self.A = tk.StringVar()
        self.A.set("[*] Host: {} | Port: {}".format(self.CLIENT_IP,self.PORT))
        ...
        self.button1 = tk.Button(self.master,text="Send",bg = 'blue' , fg='white',command=lambda: self.functionality(self.entry3.get()))
    ...
    def connectToServer(self):
        ...
        self.A.set("\nPlease choose from one of the following commands\nCREATE_USER <NAME> <USER_NAME> <PASSWORD>\nLOGIN <USER_NAME> <PASSWORD>\nJOIN <GROUP_NAME>\nCREATE <GROUP_NAME>\nLIST\nSEND <USER_NAME> <MESSAGE>\nSEND_TO_GROUP <GROUP_NAME(S)> <MESSAGE>\nSEND FILE <USER_NAME> <FILENAME>\nSEND_TO_GROUP FILE <GROUP_NAME(S)> <FILENAME>")
    ...
    def tt(self):
        ...
        self.button = tk.Button(self.master,text="ok",bg = 'blue' , fg='white',command=lambda: self.operate(self.entry3.get())).pack()
    def thread(self,input):
        print(input)
        self.x = threading.Thread(target=self.operate, args=(input,))
        self.x.start()
        self.x.join()
    def operate(self,my_input):
        ...
        if (len(my_input_list)!=0) :
            if(my_input_list[0] == "CREATE_USER") :
                ...
                #self.label1 = tk.Label(self.master, textvariable=B)
                #self.label1.pack()
                self.button.pack_forget()
    ...
    def functionality(self,my_input):
            ...
            if(my_input_list[0] == "CREATE_USER") :
                ...
                self.entr.insert(tk.END,"Invalid command" + '\n')
                print()
    ...
    def create_user(self,command_list):
        ...
        if msg =="1":
            ...
            self.entr.insert(tk.END,"User created successfully" + '\n')
        elif msg =="-1":
            #self.print_error("Error: username already exists")
            self.entr.insert(tk.END,"Error: username already exists" + '\n')
        else:
            #self.print_error("Error: wrong number of arguments")
            self.entr.insert(tk.END,"Error: wrong number of arguments" + '\n')
    ...
    def login_user(self,command_list):
        ...
        if msg == "1":
            ...
            self.entr.insert(tk.END,"logged in successfully" + '\n')
            return 1
        elif msg=="-1":
            #self.print_msg("Invalid credentials")
            self.entr.insert(tk.END,"Invalid credentials" + '\n')
            return 0
        elif msg=="-2":
            #self.print_msg("Username doesn't exist. Signup first")
            self.entr.insert(tk.END,"Username doesn't exist. Signup first" + '\n')
            return 0
        else:
            #self.print_error("Wrong arguments")
            self.entr.insert(tk.END,"Wrong Arguments" + '\n')
            return 0

    def create_group(self,command_list):
        if self.isLoggedIn == False :
            #self.print_error("Please Login First")
            self.entr.insert(tk.END,"Please Login First" + '\n')
        else :
            ...
            if(msg=="False") :
                ...
                self.entr.insert(tk.END,"Group exists! Please use JOIN command to join the group" + '\n')
            else :
                self.print_msg(command_list[1]+" group created succesfully ")
                self.entr.insert(tk.END,"Group Created Successfully" + '\n')
        ...
        if self.isLoggedIn == False :
            ...
            if(msg=="False") :
                ...
                if command_list[1] in self.my_group_list:
                    ...
                    self.entr.insert(tk.END,"Already a member" + '\n')
                else:
                    self.print_msg(str(self.current_userid)+" joined group "+str(command_list[1])+" succesfully ")
                    self.entr.insert(tk.END,"Joined Group Successfully" + '\n')
                    self.my_group_list.add(command_list[1])
                    self.user_key_pair.joingroup(command_list[1],msg)
    ...
    def send_message_msg(self,command_list):
        ...
        if msg=="0":
            ...
            self.entr.insert(tk.END,"Error:wrong arguments" + '\n')
        elif msg=="-1":
            #self.print_error("Receiver username not part of chat application")
            self.entr.insert(tk.END,"Receiver username not part of chat application" + '\n')
        else:
            ip=msg.split(" ")[0]
    ...
    def send_message_file(self,command_list):
        ...
        if msg=="False":
            ...
            self.send_message_filedata(ip,port,filename)
    ...
    def send_message_filedata(self,ip,port,filename,group=None):
        ...
        try:
            ...
            self.entr.insert(tk.END,"File sent" + '\n')
            
            
        except FileNotFoundError as f:
            self.print_error(str(f))
            self.entr.insert(tk.END,"File not found" + '\n')
    ...
    def list_group(self,command_list):
        ...
        if group_len==0:
            ...
            self.entr.insert(tk.END,"No Group" + '\n')
        else:
            self.print_msg("GROUP LIST:")
            self.entr.insert(tk.END,"GROUP LIST: " + '\n')
            for i in range(0,group_len):
                msg=self.recieve_message_decrypt(self.user_key_pair.server_key)
                self.print_msg(msg)
                self.entr.insert(tk.END,msg + '\n')
                
    def send_to_group(self,command_list):
        if(len(command_list)<3):
            #self.print_error("Invalid command")
            self.entr.insert(tk.END,"Invalid Command, Try again" + '\n')
        else:
            if command_list[1]=="FILE":
                ...
                for i in groupnames:
                    if i not in self.my_group_list:
                        ...
                        self.entr.insert(tk.END,"User not part of group" + '\n')
                    else:
                        command_list[2]=i
    ...
    def send_to_group_file(self,command_list):
        ...
        for i in range(msg1):
            ...
            self.send_message_filedata(ip,port,command_list[3],command_list[2])
    ...
    def handle_client_file(self,conn,sk):         
        try:
            ...
            self.entr.insert(tk.END,file_name +' ' + 'received' + '\n')
        except:
            self.print_error("Error in receiving file")
            self.entr.insert(tk.END,'error receiving file' + '\n')
